...
| 데이터 | 설명 | 예시 규모 |
|--------|------|-----------|
| 🗂️ **카테고리** | 상품 분류 (전자제품 > 컴퓨터 > 노트북 같은 3단계 구조) | - |
...
```
   ┌────────────────────┐       ┌──────────────────┐       ┌─────────────────────┐
   │  ① 데이터 만들기    │  →   │  ② 클라우드 준비  │  →   │  ③ 자동 배포·실행   │
   │  (파이썬 프로그램)  │       │   (Databricks)   │       │  (GitHub Actions)   │
   └────────────────────┘       └──────────────────┘       └─────────────────────┘
   회원·상품·주문 데이터를       데이터를 저장할 창고와        코드를 올리면 알아서
   현실처럼 생성                 작업할 컴퓨터를 정의          만들고 클라우드에 올림
```
...
1. **개발자가 코드를 GitHub에 올립니다** (push)
2. **GitHub이 자동으로 감지**해서 배포를 시작합니다
3. 파이썬 코드를 **설치 가능한 꾸러미(wheel)** 로 포장합니다
4. 이 꾸러미를 **Databricks 클라우드에 업로드**합니다
5. Databricks가 **작업용 컴퓨터를 켜서** 데이터를 생성합니다
6. 만들어진 데이터를 **클라우드 저장소에 차곡차곡 저장**합니다
7. 결과(성공/실패)를 **디스코드(Discord)로 알림**을 보내줍니다 📨
